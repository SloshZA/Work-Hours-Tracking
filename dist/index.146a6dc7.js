...
request.onupgradeneeded = (event)=>{
    ...
    if (event.oldVersion < 4) {
        ...
        if (!db.objectStoreNames.contains('customers')) {
            ...
            if (!customerStore.indexNames.contains('name')) customerStore.createIndex('name', 'name', {
                unique: false
            });
        }
        if (!db.objectStoreNames.contains('vehicles')) {
            ...
            if (!vehicleStore.indexNames.contains('name')) vehicleStore.createIndex('name', 'name', {
                unique: true
            });
        } else {
    ...
    if (event.oldVersion < 5) {
        ...
        if (!db.objectStoreNames.contains('reports')) {
            ...
            reportStore.createIndex('tripId', 'tripId', {
                unique: false
            });
...
request.onsuccess = (event)=>{
    ...
    if (storedActivity) try {
        activeActivityData = JSON.parse(storedActivity);
        console.log('Restored active activity from localStorage:', activeActivityData);
        if (activeActivityData.type === 'travel') displayActiveTripInfo(activeActivityData);
        else if (activeActivityData.type === 'office') displayActiveOfficeInfo(activeActivityData);
    ...
    if (userSelect) getPreference('lastUser', (lastUser)=>{
        ...
        } else console.log('No valid lastUser preference found.');
    });
    else console.warn('User select element (#userSelect) not found.');
...
function saveTrip(trip, onSuccessCallback) {
    ...
    request.onsuccess = ()=>{
        ...
        if (onSuccessCallback) onSuccessCallback(request.result); // Pass back the generated ID if needed
        // Update vehicle preference only when starting? Or always? Let's keep it here.
        if (trip.vehicle) savePreference('lastVehicle', trip.vehicle);
    };
    request.onerror = (event)=>{
    ...
    getVehicles((vehicles)=>{
        ...
        if (vehicles && vehicles.length > 0) {
            ...
            getPreference('lastVehicle', (lastVehicle)=>{
                ...
                } else console.log('No valid lastVehicle preference found or vehicle deleted.');
            });
        } else console.log('No vehicles found in database to populate dropdown.');
    });
}
// --- Modify populateCustomerDropdown to accept a callback ---
function populateCustomerDropdown(selectElementId, callback) {
    ...
    getCustomers((customers)=>{
        ...
        } else console.log('No customers found in database.');
...
function openStartTripModal(isEditing = false) {
    ...
    populateCustomerDropdown('modalCustomer', ()=>{
        ...
        } else modalCustomerSelect.value = ''; // Ensure customer is cleared for new trip
...
function openEndTripModal() {
    ...
    if (endTripModal) endTripModal.style.display = 'block';
}
function closeEndTripModal() {
    if (endTripModal) endTripModal.style.display = 'none';
}
function handleEndTripFormSubmit(event) {
    ...
    saveTrip(completedTrip, (success)=>{
        if (success) {
            ...
            populateVehicleDropdown();
        } else alert('Failed to save completed trip.');
    });
}
// --- NEW: Start Choice Modal Functions ---
function openStartChoiceModal() {
    if (startChoiceModal) startChoiceModal.style.display = 'block';
}
function closeStartChoiceModal() {
    if (startChoiceModal) startChoiceModal.style.display = 'none';
}
...
function handleCompleteOfficeWork() {
    ...
    saveTrip(completedOfficeEntry, (success)=>{
        if (success) {
        ...
        } else alert('Failed to save completed office work.');
    });
}
...
function updateVehicleKm(vehicleName, newKm) {
    ...
    getRequest.onsuccess = ()=>{
        ...
        } else console.warn(`Vehicle ${vehicleName} not found for KM update.`);
    };
...
function displayActiveTripInfo(trip) {
    ...
    } else // Log which specific element might be missing for easier debugging
...
function displayActiveOfficeInfo(officeData) {
    ...
    } else console.error('Could not find active office display elements.');
}
...
function setupEventListeners() {
    ...
    if (startNewActivityBtn) startNewActivityBtn.addEventListener('click', (event)=>{
        event.preventDefault();
    ...
    else console.warn('Button with ID "saveTripBtn" not found');
    // Complete Trip Button (Travel) - No change in listener setup
    if (completeTripBtnOnPage) completeTripBtnOnPage.addEventListener('click', openEndTripModal);
    ...
    if (startOfficeBtn) startOfficeBtn.addEventListener('click', ()=>{
        closeStartChoiceModal();
    ...
    if (editOfficeWorkBtn) editOfficeWorkBtn.addEventListener('click', handleEditOfficeWork);
    else console.warn('Button with ID "editOfficeWorkBtn" not found');
    if (completeOfficeWorkBtn) completeOfficeWorkBtn.addEventListener('click', handleCompleteOfficeWork);
    else console.warn('Button with ID "completeOfficeWorkBtn" not found');
    // Close modals if clicked outside
    window.addEventListener('click', (event)=>{
        if (event.target === startTripModal) closeStartTripModal();
        if (event.target === endTripModal) closeEndTripModal();
        // --- NEW: Add checks for new modals ---
        if (event.target === startChoiceModal) closeStartChoiceModal();
        if (event.target === officeEntryModal) closeOfficeEntryModal();
        if (event.target === workReminderModal) closeWorkReminderModal();
    });
    // Navigation Button Listeners - No change
    if (viewTripsBtn) viewTripsBtn.addEventListener('click', ()=>{
    ...
    else console.warn('Button with ID "reportsBtn" not found');
    // --- NEW: Work Schedule Button Listener ---
    if (workScheduleBtn) workScheduleBtn.addEventListener('click', ()=>{
        window.location.href = 'Work_Schedule.html'; // Navigate to the new page
    });
    else console.warn('Button with ID "workScheduleBtn" not found');
    // --- End New Listener ---
    if (manageCustomersBtn) manageCustomersBtn.addEventListener('click', ()=>{
        window.location.href = 'customers.html';
    });
    if (manageVehiclesBtn) manageVehiclesBtn.addEventListener('click', ()=>{
    ...
    if (deleteOfficeWorkBtn) deleteOfficeWorkBtn.addEventListener('click', handleDeleteOfficeWork);
    else console.warn('Button with ID "deleteOfficeWorkBtn" not found');
    // NEW: Add listener for the delete trip button
    if (deleteTripBtn) deleteTripBtn.addEventListener('click', handleDeleteTripTask);
    else console.warn('Button with ID "deleteTripBtn" not found');
    // --- NEW: Edit Trip Button Listener ---
    if (editTripBtnOnPage) editTripBtnOnPage.addEventListener('click', handleEditTripTask);
    else console.warn('Button with ID "editTripBtn" not found');
    ...
    } else console.warn('Button with ID "workReminderBtn" not found');
    ...
    } else console.warn('Button with ID "closeReminderModalBtn" not found');
    ...
    if (workReminderForm) {
        ...
        workReminderForm.addEventListener('submit', (event)=>{
            ...
            saveReminder(reminderData, (success)=>{
                ...
                } else alert('Failed to save work reminder. Please try again.');
            });
        });
    } else console.warn('Form with ID "workReminderForm" not found');
}
...
function addDefaultVehicleIfNeeded() {
    ...
    getRequest.onsuccess = ()=>{
        ...
        } else // 'Test' vehicle already exists
...
function openWorkReminderModal() {
    ...
    } else console.error('Work Reminder modal element not found.');
}
...
function setReminderAsActive(reminderId) {
    ...
    request.onsuccess = ()=>{
        ...
        try {
            ...
            if (activeTask.type === 'travel') displayActiveTripInfo(activeTask);
            else if (activeTask.type === 'office') displayActiveOfficeInfo(activeTask);
...
//# sourceMappingURL=index.146a6dc7.js.map
